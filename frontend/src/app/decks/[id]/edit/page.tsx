...
import { getDeck, getAllFlashcards, editDeck } from "@/lib/deck-api";
...
export default function EditDeckPage() {
  ...
  const [cards, setCards] = useState<
    { id?: string; front: string; back: string; starred?: boolean }[]
  >([{ front: "", back: "" }]);
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const deckData: Deck = await getDeck(id, getToken);
        const flashcardData: Flashcard[] = await getAllFlashcards(id, getToken);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const updatedDeck = await editDeck(
        id,
      ...
      let message = "Failed to update deck.";
      if (err?.message) {
        try {
